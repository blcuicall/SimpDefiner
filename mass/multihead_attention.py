...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if tgt_types is not None and isinstance(tgt_types, dict) and len(tgt_types) > 1:
            self.q_proj = nn.ModuleList(
                [nn.Linear(embed_dim, embed_dim, bias=bias) for _ in range(len(tgt_types))])
        ...
        if enable_torch_version is not None:
            self.enable_torch_version = enable_torch_version
        else:
    ...
    def reset_parameters(self):
        if self.qkv_same_dim:
            ...
            if self.tgt_types is not None:
                for q_proj in self.q_proj:
                    nn.init.xavier_uniform_(q_proj.weight, gain=1 / math.sqrt(2))
            else:
    ...
    def forward(
        ...
        if self.enable_torch_version and not self.onnx_trace and incremental_state is None and not static_kv:
            return F.multi_head_attention_forward(query,
                                                  key,
                                                  value,
                                                  self.embed_dim,
                                                  self.num_heads,
        ...
        if self.self_attention:
            ...
            k = self.k_proj(key)
            v = self.v_proj(value)
        if tgt_type is not None and self.tgt_types is not None:
            q = self.q_proj[self.tgt_types[tgt_type]](query)
        else:
            q = self.q_proj(query)
        q *= self.scaling
