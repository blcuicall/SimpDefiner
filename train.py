...
def main(args, init_distributed=False):
    if args.device_id == 0:
        wandb.init(project='def-gen-simp-final',
                   entity='cunliang-kong',
                   reinit=False)
        wandb.run.name = os.path.split(args.save_dir)[1]

        config = wandb.config
        hyper_param_of_interest = [
            'lr', 'min_lr', 'clip_norm', 'warmup_init_lr', 'warmup_updates',
            'weight_decay', 'label_smoothing', 'update_freq', 'max_tokens',
            'max_epoch', 'dropout', 'dae_styles'
        ]
        for hp in hyper_param_of_interest:
            setattr(config, hp, getattr(args, hp))
...
def train(args, trainer, task, epoch_itr):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        src_tgt, style_style = args.model_lang_pairs

        if args.device_id == 0:
            wandb_log = {
                'train/loss': stats['loss'].avg,
                'train/nll_loss': stats['nll_loss'].avg,
                'train/ppl': stats['ppl'],
                'train/lr': stats['lr'],
                'src-tgt/loss': stats[f'{src_tgt}:loss'],
                'src-tgt/nll_loss': stats[f'{src_tgt}:nll_loss'],
                'style-style/loss': stats[f'{style_style}:loss'],
                'style-style/nll_loss': stats[f'{style_style}:nll_loss'],
            }
            wandb.log(wandb_log)
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        if args.device_id == 0:
            wandb_log = {
                'valid/loss': stats['loss'].avg,
                'valid/nll_loss': stats['nll_loss'].avg,
                'valid/ppl': stats['ppl'],
            }
            wandb.log(wandb_log)
